...
class PlayerSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if pace_data:
            ...
            image_path = os.path.join(output_dir, player["name"], f'{self.pace_type}.png')
            ...
            await interaction.response.edit_message(content=f"No data available for {self.pace_type}.", view=None)
...
class PaceCommand(commands.Cog):
    ...
    @app_commands.describe(pace_type="The pace type you want to see. Either 'wins' or 'points'.", identifier="The identifier of the player you want to see the pace of.", borders="Whether or not you wish to see the border on your graph")
    @app_commands.autocomplete(pace_type=autocompletion_type_pace, borders=autocomplete_borders)
    async def pace(self, ctx, pace_type: str = "wins", identifier: str = None, borders: str = None):
        try:
            ...
            if isinstance(players, list) and len(players) > 1:
                ...
                await ctx.followup.send("Multiple players found. Please select one:", view=PlayerSelectView(players, ctx, pace_type + "_pace", borders, update))
                return
            ...
            if pace_type not in ['wins_pace', 'points_pace']:
                await ctx.followup.send(f'Invalid pace type "{pace_type}". Use "wins" or "points".')
                return
