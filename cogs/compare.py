...
class CompareCommand(commands.Cog):
    ...
    async def compare(self, ctx, type: str = None, users: str = None, ranks: str = None):
        try:
            await ctx.response.defer()
            if type not in ["points_pace", "wins_pace", "ranks", "wins", "points", "points_wins", "max_wins", "max_points"]:
                await ctx.followup.send("Please provide a valid type: 'wins', 'points', 'max_points', 'max_wins', 'wins_pace', 'points_pace', or 'ranks'.")
                return
            ...
            update = update or datetime.combine(date.fromisoformat("2005-03-30"), datetime.min.time())
    ...
    async def resolve_next_user(self, ctx, type, update):
        ...
        # Create select menu
        options = [discord.SelectOption(label=p["name"], value=str(i)) for i, p in enumerate(players)]
        ...
        class PlayerSelect(discord.ui.Select):
            def __init__(self, parent, user, players):
                ...
                super().__init__(placeholder=f"Select the correct player for {user}:", options=options)

            async def callback(self, interaction: discord.Interaction):
                selected_index = int(self.values[0])
                self.parent.resolved_users[self.user] = self.players[selected_index]
                await interaction.response.edit_message(content=f"✅ Selected **{self.players[selected_index]['name']}** for `{self.user}`.", view=None)
        ...
        await ctx.followup.send(f"Multiple players found for `{user}`. Please select the correct one:", view=view)
