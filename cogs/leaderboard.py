...
class LeaderboardCommand(commands.Cog):
    ...
    async def leaderboard(self, ctx, type: str = "points", page: int = 1):
        try:
            ...
            if type not in ["wins_pace", "points_pace", "wins", "points", "points_wins"]:
                await ctx.followup.send("Please provide a valid type. Either 'wins_pace', 'points_wins', 'points_pace', 'wins' or 'points'.")
                return
            players_per_page = 10
            ...
            if not lb:
                embed = discord.Embed(
                    ...
                    description="No players found for the current leaderboard criteria.",
            ...
            embed = discord.Embed(
                title="🏆 Top Players Leaderboard 🏆",
                description=f"Here are the players ranked {offset + 1} to {min(offset + players_per_page, total_players)} with the highest {type}.",
