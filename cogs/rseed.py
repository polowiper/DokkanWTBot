...
class RandomSeedCommand(commands.Cog):
    ...
    @app_commands.command(
        name='rseed',
        description=
        "Show the seed performance of a player relative to a certain amount of resets."
    )
    @app_commands.describe(identifier="Player identifier (name/ID).",
                           resets_amount="Number of resets for calculation.")
    async def rseed(self,
                    ctx,
                    identifier: str = None,
                    resets_amount: int = None):
        try:
            ...
            current_hour = round(elapsed.days * 24 + elapsed.seconds / 3600,
                                 2) if elapsed else 71.26
            ...
            for player in seed_lb:
                ...
                if len(player_points) < resets_amount or len(
                        player_wins) < resets_amount:
                    wins_diff = player_wins[-1] - player_wins[0]
                    player_relative_seed = int(
                        (player_points[-1] - player_points[0]) /
                        wins_diff) if wins_diff != 0 else 0
                else:
            ...
            embed = discord.Embed(
                ...
                description=
                f"Here are the top 10 players with the highest relative seed over the last {resets_amount} resets.",
            ...
            for idx, player in enumerate(top_10_list, start=1):
                ...
                if player[0] == player_name:
                    embed.add_field(name=f"🔍 {rank_emoji} {player[0]} (You!)",
                                    value=f"Relative Seed: **{player[1]}**",
                                    inline=False)
                else:
                    embed.add_field(name=f"{rank_emoji} {player[0]}",
                                    value=f"Relative Seed: **{player[1]}**",
                                    inline=False)

            if index is not None and player_name not in [
                    p[0] for p in top_10_list
            ]:
                player = player_list[index]
                embed.add_field(
                    name="🔍 Player Not in Top 10",
                    value=
                    f"**{player[0]}** is ranked **#{index+1}** with a Relative Seed of **{player[1]}**.",
                    inline=False)

            embed.set_footer(
                text=
                f"Displaying leaderboard for relative seed. Hope you're in the top 10!"
            )
            embed.add_field(
                name="Updated at",
                value=
