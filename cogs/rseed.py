...
class RandomSeedCommand(commands.Cog):
    ...
    @app_commands.describe(identifier="Player identifier (name/ID).", resets_amount="Number of resets for calculation.")
    async def rseed(self, ctx, identifier: str = None, resets_amount: int = None):
        try:
            ...
            if identifier is None:
                ...
                if identifier is None:
                    await ctx.followup.send("You did not provide a Dokkan name/ID and your Discord account isn't linked. Please provide an identifier or link your Discord account (/link).")
                    return
            
            current_hour = round(elapsed.days * 24 + elapsed.seconds / 3600, 2) if elapsed else 71.26
            ...
            if not seed_lb:
                await ctx.followup.send("There's a problem with the database chief, We might be cooked. Ask Polo to fix it.")
                return
            ...
            for player in seed_lb:
                ...
                if len(player_points) < resets_amount or len(player_wins) < resets_amount:
                    wins_diff = player_wins[-1] - player_wins[0]
                    player_relative_seed = int((player_points[-1] - player_points[0]) / wins_diff) if wins_diff != 0 else 0 
                else:
                    wins_diff = player_wins[-1] - player_wins[-resets_amount]
                    player_relative_seed = int((player_points[-1] - player_points[-resets_amount]) / wins_diff) if wins_diff != 0 else 0 
            ...
            embed.set_footer(text=f"Displaying leaderboard for relative seed. Hope you're in the top 10!")
