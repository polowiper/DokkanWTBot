...
def plot_and_save(players, key, xlabel, ylabel, title, output_dir, step=False, animate=False, multiple=False, border=None):
    if multiple:
        ...
        if border:
            for b in border:
                b_name = b['rank']
                border_line, = ax.plot(b['hour'], b[key], linestyle='dotted', label=f"{b_name} (border)")
        ...
        for player in players:
            ...
            line, = ax.step(player['hour'], player[key], where='mid') if step else ax.plot(player['hour'], player[key])
            
            if border:
                for b in border:
                    b_name = b['rank']
                    ax.plot(b['hour'], b[key], linestyle='dotted', label=f"{b_name} (border)")
...
def render(players, output_dir, type="all", animate=False, multiple=False, bulk_params=None, step=False, border=None):
    if type == "all":
        for key in key_map.keys():
            title, ylabel = key_map[key]
            plot_and_save(players, key, 'Hours', ylabel, title, output_dir, animate=animate, multiple=multiple, step=step, border=border)
    elif type == "bulk":
        ...
        plot_and_save(players, type, 'Hours', ylabel, title, output_dir, animate=animate, multiple=multiple, step=step, border=border)
